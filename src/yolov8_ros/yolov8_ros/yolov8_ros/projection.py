...
with open(f'./testing/calib/{name}.txt','r') as f:
    calib = f.readlines()
# print(calib[2]) #P2 -->calib_cam_to_cam: P_rect_02
# print(calib[4]) #R0_rect -->calib_cam_to_cam: R_rect_00
# print(calib[5]) #Tr_velo_to_cam -->calib_imu_to_velo: R
# P2 (3 x 4) for left eye
P2 = np.array([float(x) for x in calib[0].strip('\n').split(' ')[1:]]).reshape(3,4)
R0_rect = np.array([float(x) for x in calib[1].strip('\n').split(' ')[1:]]).reshape(3,3)
# Add a 1 in bottom-right, reshape to 4 x 4
R0_rect = np.insert(R0_rect,3,values=[0,0,0],axis=0)
R0_rect = np.insert(R0_rect,3,values=[0,0,0,1],axis=1)
Tr_velo_to_cam = np.array([float(x) for x in calib[2].strip('\n').split(' ')[1:]]).reshape(3,4)
